...
public class SLRParser {
    ...
    public ParserReport buildReport() {
        ...
        LR0Automaton automaton = Items.buildLR0Automaton(grammar);
    ...
    private ParsingTable buildParsingTable(LR0Automaton automaton) {
        ...
        for (int state = 0; state < automaton.states.size(); state++) {
            Set<LR0Item> items = automaton.states.get(state);
            for (LR0Item item : items) {
                ...
                if (item.isComplete()) {
                    if (item.lhs.equals(augmentedStart)) {
                        ...
                        Set<String> follow = grammar.getFollowSets().getOrDefault(item.lhs, Set.of());
                        for (String terminal : follow) {
                            table.addReduce(state, terminal, findProduction(item));
                        }
                    }
                }
            }
    ...
    private Production findProduction(LR0Item item) {
    ...
    private String formatAutomaton(LR0Automaton automaton) {
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < automaton.states.size(); i++) {
            builder.append(Items.formatLR0ItemSet(i, automaton.states.get(i)));
